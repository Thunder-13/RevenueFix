...
interface NetworkBillingData {
  summary: {
    total_network_records: number;
...
const NetworkBilling = () => {
  const [data, setData] = useState<NetworkBillingData | null>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await apiService.getNetworkBillingData();
        setData(response.data.data);
      } catch (error) {
        console.error("Error fetching network billing data:", error);
        toast({
          title: "Error",
          description: "Failed to load network billing data. Please try again.",
          variant: "destructive",
        });
      } finally {
        setLoading(false);
      ...
      <div className="flex min-h-screen">
        ...
        <div className="flex flex-1 flex-col">
          ...
          <main className="flex flex-1 items-center justify-center">
            <LoadingSpinner size="lg" text="Loading network billing data..." />
          </main>
        </div>
      </div>
    );
      ...
      <div className="flex flex-1 flex-col">
        <main className="flex-1 p-6 md:p-8">
          <div className="mx-auto max-w-7xl">
            <h1 className="mb-6 text-3xl font-bold">Network vs Billing</h1>
